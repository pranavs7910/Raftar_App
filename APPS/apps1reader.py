"""
APPS1 Accelerator Pedal Position Sensor - Data Analysis Script
Analyzes raw 12-bit ADC values from apps1_app.txt
"""
...
V_MIN           = 0.0           # APPS1 valid range: lower bound (V)
V_MAX           = 1.68          # APPS1 valid range: upper bound  (V)
MOVING_AVG_WIN  = 10            # Moving-average window size (samples)
FILE_NAME       = "apps1_app.txt"

# Derived ADC limits from voltage range
ADC_MIN = round((V_MIN / V_REF) * (ADC_RESOLUTION - 1))   # → 0
ADC_MAX = round((V_MAX / V_REF) * (ADC_RESOLUTION - 1))   # → ~2083
...
def moving_average(data: np.ndarray, window: int) -> np.ndarray:
    ...
    The first (window-1) samples are averaged over however many points exist."""
...
def plot_results(adc_raw: np.ndarray,
    ...
    time_axis = np.arange(len(adc_raw))   # sample index as "time"

    fig = plt.figure(figsize=(14, 8), facecolor="#0f1117")
    fig.suptitle("APPS1 – Accelerator Pedal Position Sensor Analysis",
    ...
    ax_style = dict(facecolor="#1a1d27",
                    tick_params=dict(colors="#aaaaaa", labelsize=9))
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    # Locate the data file (same dir as script, or cwd)
    ...
    print(f"\n{'═'*46}")
    print(f"  APPS1 Sensor Log Analysis")
    print(f"{'═'*46}")
